...
class Game:
    ...
    def create_random_background(self, width, height, floor_tiles):
        ...
        title_w = floor_tiles[0].get_width()
        title_h = floor_tiles[0].get_height()

        for y in range(0, height, title_h):
            for x in range(0, width, title_w):
                title = random.choice(floor_tiles)
                bg.blit(title, (x, y))
        return bg
